...
class RTCPeerConnection extends EventEmitter
{
	...
	_handleRtpSenders()
	{
		this._logger.debug('_handleRtpSenders()');
		...
		{
			let promise = this._handleRtpSender(rtpSender, transport);
	...
	createAnswer()
	{
		...
		// Refresh out RtpSenders.
		return this._handleRtpSenders()
			.then(() =>
			{
				this._logger.debug('createAnswer() | succeed');
	...
	setRemoteDescription(desc)
	{
		...
		if (!RTCSessionDescription.isDescription(desc))
			return Promise.reject(new TypeError('invalid RTCSessionDescription'));
		...
		try
		{
			...
			this._remoteSdpObject = sdpObject;
		}
		catch (error)
		{
		...
		{
			if (this._peer)
				return Promise.reject(new Error('renegotiation from endpoint not yet implemented'));
	...
	_handleRtpSender(rtpSender, transport)
	{
		...
		if (this._rtpHandlers.has(id))
			return Promise.resolve();
		if(this._remoteSdpObject)
		{
			for(let media of this._remoteSdpObject.media)
			{
				if(media.type == rtpSender.kind)
				{
					id = media.mid;
					rtpSender.adapterMuxId = id;
					break;
				}
			}
		}
	...
	{
                this._logger.debug('_checkNegotiationNeeded() _busy %s, _signalingState %s', this._busy, this._signalingState);
	...
	_createLocalDescriptionUnifiedPlan(type)
	{
		...
		var sortedkeys = Array.from(this._rtpHandlers.keys());
		sortedkeys.sort();
		obj.groups =
		[
			{
				type : 'BUNDLE',
				mids : sortedkeys.filter((mid) =>
					{
		...
		{
			let rtpHandler = this._rtpHandlers.get(key);
			let objMedia = {};
			...
			// mid.
			objMedia.mid = rtpHandler.adapterMuxId ? rtpHandler.adapterMuxId : rtpHandler.rtpParameters.muxId;
